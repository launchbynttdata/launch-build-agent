...
remote_state {
  ...
  config = {
    profile        = "${local.profile_name}"
    bucket         = "${replace(local.inputs.naming_prefix, "_", "-")}-gitwebhooks-${local.region}-${local.account_name}-${local.environment_number}-tfstate-000"
    key            = "terraform.tfstate"
    region         = "${local.region}"
    encrypt        = true
    dynamodb_table = "${local.inputs.naming_prefix}-gitwebhooks-${local.region}-${local.account_name}-${local.environment_number}-tflocks-000"
  }
}
